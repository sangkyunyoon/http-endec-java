 ...
 * 
 */
public class HttpRequestFrameTest {
	...
	public void toStringTest() {
		...
		frame = new HttpFrame("POST", version, headers,
				"/rest/help/todo", new ListOfBytesMock(
						"kind of body\r\ntestCarriage"));
	...
	public void parseRequestLineTest() {
		...
		try {
			try {
				assertTrue(
						"request is OK",
						new HttpFrame().parseRequestLine(inputstream) == 200);
			} catch (IOException e) {
				fail("IOException");
			}
		...
		// invalid http header
		basicHttpHeader = "invalid http request/response";
		...
		try {
			try {
				assertTrue(
						"request is invalid",
						new HttpFrame().parseRequestLine(inputstream) == 400);
			} catch (IOException e) {
				fail("IOException");
			}
		} catch (InterruptedException e) {
		...
		try {
			try {
				assertTrue(
						"bad request or invalid inputstream",
						new HttpFrame().parseRequestLine(inputstream) == 500);
			} catch (IOException e) {
				fail("IOException");
			}
		} catch (InterruptedException e) {
		...
		// post valid request 
		basicHttpHeader = "POST /rest/fake HTTP/1.1";
		inputstream = new ByteArrayInputStream(basicHttpHeader.getBytes());
		...
		basicHttpHeader = "POS1T /rest/fake HTTP/1.1";
		inputstream = new ByteArrayInputStream(basicHttpHeader.getBytes());
		...
		try {
			try {
				assertTrue("request is POST and invalid",
						frame.parseRequestLine(inputstream) == 400);
			} catch (IOException e) {
				fail("IOException");
			}
		} catch (InterruptedException e) {
		...
		// get valid request
		basicHttpHeader = "GET /rest/fake HTTP/1.1";
		inputstream = new ByteArrayInputStream(basicHttpHeader.getBytes());
		...
		try {
			try {
				assertTrue("request is GET and valid",
						frame.parseRequestLine(inputstream) == 200);
				assertTrue("request is GET and valid", frame.getMethod()
						.equals("GET"));
				assertTrue("request is GET and valid",
						frame.getUri().equals("/rest/fake"));
			} catch (IOException e) {
		...
		// get valid request
		basicHttpHeader = "HTTP/1.1 /test/fake";
		inputstream = new ByteArrayInputStream(basicHttpHeader.getBytes());
		...
		try {
			try {
				assertTrue("request is beginning with http version",
						frame.parseRequestLine(inputstream) == 200);
				assertTrue("request is beginning with http version", frame
						.getUri().equals("/test/fake"));
			} catch (IOException e) {
				fail("IOException");
			}
		} catch (InterruptedException e) {
		...
		// get valid request
		basicHttpHeader = "HTTP/1 /test/fake";
		inputstream = new ByteArrayInputStream(basicHttpHeader.getBytes());
		...
		try {
			try {
				assertTrue("request is beginning with wrong http version",
						frame.parseRequestLine(inputstream) == 400);
			} catch (IOException e) {
				fail("IOException");
			}
	...
	*/
	...
	public void parseBodyTest() {
		...
		try {
			assertTrue(
					"test basic body with no content-length",
					new HttpFrame().parseBody(inputstream).getBytes().length == 0);
		} catch (IOException e) {
			fail("IOException");
		}
		HashMap<String, String> headers = new HashMap<String, String>();
		headers.put("content-length", String.valueOf(basicBody.length()));
		HttpFrame frame = new HttpFrame();
		frame.setHeaders(headers);
		inputstream = new ByteArrayInputStream(basicBody.getBytes());
		try {
			assertTrue("test basic body with content-length",
					frame.parseBody(inputstream).getBytes().length == basicBody
							.length());
		} catch (IOException e) {
			fail("IOException");
		}
		...
		try {
			assertTrue(
					"test basic body with content-length of modulo 4089 ==0",
					frame.parseBody(inputstream).getBytes().length == basicBody
							.length());
		} catch (IOException e) {
			fail("IOException");
		...
		inputstream = new ByteArrayInputStream(basicBody.getBytes());
		try {
			assertTrue("test basic body with content-length > 4089",
					frame.parseBody(inputstream).getBytes().length == basicBody
							.length());
		} catch (IOException e) {
			fail("IOException");
	...
	public void parseHttpTest() {
		...
		try {
			try {
				assertTrue("test valid post request",
						new HttpFrame().parseHttp(inputstream) == 200);
			} catch (InterruptedException e) {
				fail("InterruptedException");
			}
		} catch (IOException e) {
		...
		try {
			try {
				assertTrue("test invalid request",
						new HttpFrame().parseHttp(inputstream) == 400);
			} catch (InterruptedException e) {
				fail("InterruptedException");
			}
		} catch (IOException e) {
		...
		try {
			try {
				assertTrue("test invalid content length (not present)",
						new HttpFrame().parseHttp(inputstream) == 400);
			} catch (InterruptedException e) {
				fail("InterruptedException");
			}
		...
		try {
			try {
				assertTrue("test invalid content length",
						new HttpFrame().parseHttp(inputstream) == 400);
			} catch (InterruptedException e) {
				fail("InterruptedException");
			}
	...
	public void parseHeaderTest() {
		// test headers size 
		String basicHttpHeader = "value1: header1\r\nvalue2: headers2";
		InputStream inputstream = new ByteArrayInputStream(
				basicHttpHeader.getBytes());

		try {
			assertTrue("test headers size",
					new HttpFrame().parseHeader(inputstream).size() == 2);
		} catch (IOException e) {
			fail("IOException");
		}
		// test headers value
		...
		try {
			assertTrue(
					"test headers value1",
					new HttpFrame().parseHeader(inputstream)
							.get("value1").toString().equals("header1"));
		} catch (IOException e) {
			fail("IOException");
		}
		inputstream = new ByteArrayInputStream(basicHttpHeader.getBytes());
		try {
			assertTrue(
					"test headers value2",
					new HttpFrame().parseHeader(inputstream)
							.get("value2").toString().equals("headers2"));
		} catch (IOException e) {
			fail("IOException");
		}
		// test host headers
		...
		HttpFrame frame = new HttpFrame();
		try {
			frame.parseHeader(inputstream);
		} catch (IOException e1) {
			fail("IOException");
		}
		assertTrue("test headers value2",
				frame.getHost().equals("www.google.com"));
		try {
			inputstream.close();
		} catch (IOException e) {
			e.printStackTrace();
